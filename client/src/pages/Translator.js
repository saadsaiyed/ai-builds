...
// --- Helper: Free Translation Service (MyMemory API + Fallbacks) ---
const translateFree = async (text, source, target) => {
  if (!text || !target) return null;

  // 1. Setup for MyMemory
  // MyMemory requires a pair like 'en|fr'. If source is auto, we default to 'en' or guess,
  // but using a fallback service is better for 'auto'.
  const mmSource = source === 'auto' ? 'en' : source;
  const email = 'demo_user_gemini@gmail.com'; // Adding email increases quota significantly (50k chars/day)

  try {
    const url = `https://api.mymemory.translated.net/get?q=${encodeURIComponent(text)}&langpair=${mmSource}|${target}&de=${email}`;
    ...
    if (data.responseStatus === 200) {
      ...
      const alternatives = (data.matches || [])
        ...
        .filter((t, i, arr) => {
          return (
            t && 
            t.toLowerCase() !== mainTranslation.toLowerCase() && 
            arr.indexOf(t) === i
          );
        })
        .slice(0, 3);
      return { text: mainTranslation, alts: alternatives };
    }
    // If not 200, throw to trigger fallback
    throw new Error(`MyMemory Status: ${data.responseStatus}`);
  } catch (err) {
    console.warn("Primary API failed, trying fallback...", err);
  }

  // 2. Fallback: Lingva Scraper (Unofficial)
  // Useful because it handles 'auto' better and has different rate limits.
  try {
    // Lingva uses 2-letter codes mostly standard.
    const lingvaSource = source === 'auto' ? 'auto' : source; 
    const url = `https://lingva.ml/api/v1/${lingvaSource}/${target}/${encodeURIComponent(text)}`;
    const res = await fetch(url);
    const data = await res.json();

    if (data.translation) {
      return { text: data.translation, alts: [] }; // Lingva doesn't provide alternatives easily
    }
  } catch (err) {
    console.warn("Fallback API failed", err);
  }

  return { text: "Limit Reached", alts: [] };
};
...
export default function Translator() {
  ...
  // Debounce at 1000ms for free API friendliness
  const debouncedText = useDebounce(inputText, 1000);

  // --- Main Translation Effect (Using Free Service) ---
  useEffect(() => {
    const performTranslation = async () => {
      ...
      if (sourceLang === 'auto') {
        setDetectedSource("Detecting...");
      } else {
        const langName = LANGUAGES.find(l => l.code === sourceLang)?.name;
        setDetectedSource(langName);
      }

      try {
        // Parallel requests now that we have better error handling/fallback
        const [res1, res2] = await Promise.all([
          translateFree(debouncedText, sourceLang, targetLang1),
          translateFree(debouncedText, sourceLang, targetLang2)
        ]);

        if (res1.text === "Limit Reached" && res2.text === "Limit Reached") {
             setError("Daily free limit reached. Try again later.");
        }
        ...
        // Update detected source if available from 'auto'
        if (sourceLang === 'auto') {
           setDetectedSource("Auto");
        }

      } catch (err) {
        console.log("Translation failed");
        setError("Service busy.");
      } finally {
        setIsLoading(false);
      }
    };
  ...
  // --- Selection / Hover Translation Effect (Using Free Service) ---
  useEffect(() => {
    const handleSelection = async () => {
      ...
      try {
        ...
        const [res1, res2] = await Promise.all([
          translateFree(selectedText, 'auto', targetLang1),
          translateFree(selectedText, 'auto', targetLang2)
        ]);
        ...
        setPopup(prev => ({
          ...
          data: {
            detectedSource: "Auto", 
            results: [
              { langName: l1Name, translations: [res1.text, ...res1.alts].slice(0, 3) },
              { langName: l2Name, translations: [res2.text, ...res2.alts].slice(0, 3) }
            ]
          }
        }));
      } catch (err) {
